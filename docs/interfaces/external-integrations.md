...
**Direction** Outbound. **Trigger** A person types into the street field of an address form, on a back-office screen or
on a public page.

**Provider.** The place service of the commercial mapping provider, reached at the fixed base address
`https://maps.googleapis.com/maps/api/place`, with two operations: the suggestion operation `/autocomplete/json` and the
detail operation `/details/json`.

**Data exchanged.** Outbound, for a suggestion: the registered key, the partly typed address, the answer fields limited
to the formatted address and the name, the input kind set to a text query, the result kind restricted to addresses, and,
when they are known, the two-letter country restriction and the language code. Outbound, for a detail: the key, the
place reference of the chosen suggestion and the answer fields limited to the address components. Both carry a **session
reference** generated by the client, which is what makes the whole lookup, however many keystrokes it took, count as one
billed request. Inbound, for a suggestion: a list of predictions, each with a description and a place reference.
Inbound, for a detail: the address components and a marked-up form of the address.

**Component mapping.** The components the provider returns are translated to the platform's address fields with this
table; the first component that claims a field wins, and the components are considered in the order of the second
column.

| Provider component | Platform field | Order |
|---|---|---|
| `country` | country | 1 |
| `street_number` | house number | 2 |
| `locality` | city | 3 |
| `postal_town` | city, when the country uses it instead of a locality | 4 |
| `route` | street | 5 |
| `postal_code` | postal code | 6 |
| `administrative_area_level_1` | state, otherwise city | 7 |
| `administrative_area_level_2` | state, otherwise city | 8 |
| `sublocality_level_1` | second street line | after the ordered ones |

The country is resolved by its two-letter code; the state is resolved inside that country by its code or by a
case-insensitive match on its name, and only when exactly one matches; the city is resolved by name inside the country,
and when it resolves and no state was found, the state of the city is taken.

**House number recovery.** When the provider returns no house number, one is guessed from what the person typed: the
postal code, the street and the city are removed from the typed text, the remainder is cut at the first comma and
trimmed, and that is used as the number. When the provider does return a number, the platform compares its own
assembled "number street" with the marked-up form the provider sent and keeps the longer of the two, because the
provider sometimes abbreviates the street name to its initials and shortens the street type, returning a form shorter
than the one assembled from the components.

**Thresholds and limits.** No call is made until the typed text is longer than the parameter
`google_address_autocomplete.minimal_partial_address_size`, default 5 characters. The call times out after 2.5 seconds.

**Authentication.** The registered key of the place service, stored in the parameter
`google_address_autocomplete.google_places_api_key`. The two endpoints that serve the browser are public, and the key
never reaches the browser: the platform calls the provider itself.

**Failure handling.** A timeout or an unreadable answer yields an empty suggestion list, or an empty address for a
detail, and is logged; the person simply types the address by hand. An answer carrying an error message is logged and
treated the same way. A detail answer without address components yields an empty address.

**Idempotency.** Read-only at the provider.

**Configuration.** The key parameter, and optionally the minimal length parameter. Without a key the feature is off and
the address fields behave as plain text.

**Records written.** None by the integration itself; the values are proposed to the form and are written only when the
person saves the record.
...
| Address autocompletion | Outbound | Key for the place service of the commercial provider | Read-only; one session reference per lookup |
| Network position of a visitor | None; a shipped database file | Not applicable | Read-only |
| Publisher account services | Outbound, browser redirection | Installation identifier and signed state | Read-only |
...
1. **Address autocompletion was missing.** The plan for this document lists address autocompletion among the
   integrations, and the system exposes two endpoints for it
   ([`endpoint-catalog.md`](endpoint-catalog.md), the address autocomplete group), but neither draft specified the
   contract. It is now specified in full, including the component mapping, the house-number recovery rule, the minimum
