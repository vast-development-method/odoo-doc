...
| `_retrieve_taxes` | internal rule | self, record, line_values, tax_type, tax_exigibility | `account_edi_ubl_cii` |  | Retrieve the taxes on the document line at import.  In a UBL/CII xml, the Odoo "price_include" concept does not exist. Hence, first look for a price_include=False, if it is unsuccessful, look for a price_include=True. |
