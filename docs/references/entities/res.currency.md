...
| `_get_simple_currency_table` | preparation rule | self, companies | `account` |  | Helper creating the currency table and returning its definition for basic cases of the system reports needing to convert amounts using only the current rates, in a single period. |
