...
| `_notifydb` | internal rule | self | `base` | model | Wake up the cron workers The SYSTEM_NOTIFY_CRON_CHANGES environment variable allows to force the notifydb on both IrCron modification and on trigger creation (regardless of call_at) |
