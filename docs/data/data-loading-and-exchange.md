...
3. **A row exists but its recorded entity differs from the entity being written.** Refuse with "For external id " the complete identifier " when trying to create/update a record of model " the transport name of the entity being written " found record of different model " the transport name the row records " (" the numeric identifier of the registry row ")".
...
| The refusal for an identifier that already names a record of another entity | The working branch described the refusal of the record loader in words. | The declared text is reproduced verbatim in section 3.2, step 3, with each placeholder described in words. |
