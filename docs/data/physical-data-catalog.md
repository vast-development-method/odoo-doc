...
| The declared maximum length of a text column | Counted the bounded text columns by their maximum: 49 columns spread over 23 distinct maxima, with two columns at eight characters, five at three, two at sixteen and one at thirty-two. | Regenerated from the observed schema: 52 bounded columns over the same 23 maxima, with four columns at eight characters, four at three, three at sixteen and two at thirty-two; every other count is identical. Section 3 now carries both the count per maximum and the name of each of the 52 columns with its table and its entity, because the maximum is part of the column a replacement must create: a value longer than the maximum is truncated on write rather than refused, so an unbounded replacement column keeps text the original would have shortened. |
