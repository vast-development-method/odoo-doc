...
| Payment QR-code (`qr_code_method`) | selection | Not copied. Which quick response code generator to use when printing. The choices are the generators available on the installation, ordered by their declared priority. Empty means "the first available generator that can produce a code". |
| Display QR-code (`display_qr_code`) | boolean | Computed, not stored. True when the type is a customer invoice, a sales receipt, a vendor bill or a purchase receipt and the company enables payment codes. |
| Display Link QR-code (`display_link_qr_code`) | boolean | Computed, not stored. Same type condition with the company's portal-link code setting. |
