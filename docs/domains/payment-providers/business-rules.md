...
**PAY-RULE-004. The company of a provider with transactions is frozen.** Changing `company` is refused as soon as at least one Payment Transaction references the provider.
...
**PAY-RULE-012. Per-connector currency restrictions.** Several connectors constrain `available_currencies` beyond the generic mechanism:
...
**PAY-RULE-024. A token never belongs to the public contact.** Evaluated whenever `partner` changes.
...
**PAY-RULE-058. Nothing to pay.** When `amount_maximum` is zero or negative the wizard shows `There is nothing to be paid.`

**PAY-RULE-059. Positive amount.** When `amount` is zero or negative the wizard shows `Please set a positive amount.`

**PAY-RULE-060. Amount within the maximum.** When `amount` exceeds `amount_maximum` the wizard shows `Please set an amount lower than %s.` with the maximum formatted in the currency.
...
**PAY-RULE-078. The contact's country must be allowed.** When the contact has a country, providers whose `available_countries` is not empty and does not contain it are removed. Reason: `incompatible country`.
...
**PAY-RULE-080. The currency must be allowed.** When a currency is known, providers whose `available_currencies` is not empty and does not contain it are removed. Reason: `incompatible currency`.
...
**PAY-RULE-083. The website must match.** When a website is known, providers whose `website` is set and different are removed. Reason: `incompatible website`.
...
**PAY-RULE-088. The contact's country must be supported by the method.** When the contact has a country, methods whose `supported_countries` is not empty and does not contain it are removed. Reason: `incompatible country`.

**PAY-RULE-089. The currency must be supported by the method.** When a currency is known, methods whose `supported_currencies` is not empty and does not contain it are removed. Reason: `incompatible currency`.
...
**PAY-RULE-106. Token scoping.** Two rules combine. A public, portal or internal user may only see tokens whose `partner` equals their own contact. Every user may only see tokens whose company is the active company or an ancestor. Billing users are granted an additional rule whose domain is always true, which lifts the contact restriction for them.
