...
**Brand.** A payment method whose `primary_payment_method` is filled. A brand is never selectable on the payment form; its only role is to display its logo next to its primary method and to carry its own support flags.
...
**Child transaction.** A transaction created from another one to capture part of it, void part of it, or refund part of it. Its `source_transaction` points at the transaction it came from.
...
**Primary payment method.** A payment method whose `primary_payment_method` is empty. Only primary methods appear on the payment form.
