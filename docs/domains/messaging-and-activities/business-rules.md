...
| `MSG-210` | Creating an Activity for somebody other than the acting user notifies that person, rendered in the assignee's language, and subscribes their contact to the related record. The notification is suppressed when the quick-update switch is on. | subject ""<record name>: <summary>" assigned to you"; subtitles "Activity: <type name>" (falling back to "Todo") and "Deadline: <due date in the recipient's date format>" |
