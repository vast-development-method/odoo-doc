...
| `invoice_reference_model` | Selection | Required. Three values: **Full Reference** — the whole document number, for example `INV/2024/00001`, whose stored value is the short name of the platform vendor and is therefore not reproduced here; `euro` — **European**, the structured creditor reference, for example `RF83INV202400001`; `number` — **Numbers only**, for example `202400001`. Default: the first value whose stored string begins with the lowercase country code of the company, otherwise the full-reference value. |
