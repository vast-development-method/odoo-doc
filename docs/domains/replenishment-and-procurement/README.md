...
9. Lead time and date planning: scheduled dates are computed backwards from the need date using rule lead times, vendor lead time, days to purchase, manufacturing lead time and days to prepare the manufacturing order; deadlines propagate along chains and delay alerts flag late origin documents.
...
12. Forecast reporting: the forecasted stock report reconciles every outgoing move with reserved stock, free stock, in-transit stock and incoming moves, in priority and date order, and lets a user reserve or unreserve the linked origin moves for a specific outgoing document.
...
| `state-machines.md` | Every state field of the domain: the archive flag and the six applicability flags of Route; the archive flag, action, supply method, push mode, destination-location origin, cancellation propagation and shipping-method propagation of Stock Rule; the trigger, snooze state, archive flag, replenishment state, manual override and two supply-column flags of Reordering Rule; the replenishment report line; the lead-time, quantity-to-order and deadline computation states; the Reference between stock documents; the procurement request; the scheduler run, its batches and the fate of one rule inside a batch; the supply method, driven status changes and lateness of Stock Move; the receipt status, shipped flag and arrival date of a Purchase Order; the drop shipping operation types, route and rules; the three warehouse flags that archive and unarchive rules; the transient wizards, with the historic period and tab flags of the Replenishment Information dialog, the snooze preset and the variant flag of the routes-diagram dialog; and the two request-for-quotation grouping selections contributed to Contact. States, transitions, guards, refusal messages and a diagram per machine, followed by a summary of every state field and an enumeration of every remaining field that carries no state. |
