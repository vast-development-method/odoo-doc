...
| `state-machines.md` | Every state field of the domain: the archive flags of Route, Stock Rule and Reordering Rule; the action, supply method and push mode of Stock Rule; the trigger, snooze state, replenishment state and manual override of Reordering Rule; the replenishment report line; the lead-time, quantity-to-order and deadline computation states; the Reference between stock documents; the procurement request; the scheduler run, its batches and the fate of one rule inside a batch; the supply method, driven status changes and lateness of Stock Move; the receipt state of a Purchase Order; the drop shipping operation types, route and rules; the three warehouse flags that archive and unarchive rules; the transient wizards. States, transitions, guards, refusal messages and a diagram per machine. |
