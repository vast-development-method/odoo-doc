...
5. The first three rows of the returned list are also returned separately, for the podium. Without a name search they are the first three of the full ranking; with a name search they are the first three matching rows.
6. Pagination shows 30 visitors per page, with at most 5 page links, over `⌈number of returned rows ÷ 30⌉` pages. When the current visitor appears in the ranking and no page was requested, the page containing that visitor is opened, computed as `⌈position of that visitor ÷ 30⌉`, and the page is scrolled to that row; when the reader is not ranked and no page was requested, page 1 is opened. The page is computed from the full-ranking position while the rows are cut from the filtered list, which is the **compatibility finding** recorded in [tracks-and-agenda.md](tracks-and-agenda.md#12-leaderboard).
...
4. **Leaderboard podium and pager.** Section 13 was corrected against the source: the podium holds
   the first three rows of the returned list rather than of the full ranking, and the page opened for
   the reader is computed from the full-ranking position while the rows are cut from the filtered
   list. The consequence is recorded as a compatibility finding in
   [tracks-and-agenda.md](tracks-and-agenda.md#12-leaderboard).
